...
class PGTestCase(unittest.TestCase):
    def test_PG(self):
        pg = PG(f, g)
        pg.generate_samples(y, x0, q_init, r_init, prior_a, prior_b, n_particles=N, ancestor_sampling=True,
                            max_iter=max_iter)
        x = pg.get_states()
        q_est = pg.get_q()
        r_est = pg.get_r()
